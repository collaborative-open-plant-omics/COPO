...
from dal import cursor_to_list, cursor_to_list_str2
    ...
    for i in profile_page:
        ...
        if i.get("associated_type", ""):
            ...
            additional_info_dict = {"associated_type_count": associated_type_count,
                                    "associated_type_columnCount": associated_type_columnCount,
                                    "associated_type_width": associated_type_width,
                                    "panel_content_paddingTop": "0px"}
            i.update(additional_info_dict)

            # Create a dictionary of associated type abbreviation and associated_type
            element_lst = []
            for associated_type in i.get("associated_type", ""):
                # Extract parentheses and its enclosed value from associated type
                abbreviation = re.search(regex, associated_type).group(1) if re.search(regex,
                                                                                       associated_type) else associated_type

                element_lst.append({"associated_type": associated_type, "abbreviation": abbreviation})

            associated_type_elements = {
                "associated_type_elements": element_lst}
...
#             with open(os.path.join(full_path, "copo_profile_index.html"), 'r') as f:
#                 html = f.read()
