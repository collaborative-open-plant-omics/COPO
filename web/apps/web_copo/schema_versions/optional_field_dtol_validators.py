...
class DtolEnumerationValidator(Validator):
    ...
    def validate(self):
        ...
        # ignore validation if column is an optional column for
        # POP_GENOMICS associated type for ERGA manifest
        if p_type == "ERGA" \
                and any(x in self.data.columns for x in lookup.POP_GENOMICS_OPTIONAL_COLUMNS_DEFAULT_VALUES_MAPPING):
            pass
        else:
